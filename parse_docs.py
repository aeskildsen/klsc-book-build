...
MKDOCS_ROOT_FOLDER = './komposition-og-lydproduktion-med-supercollider/'
...
def preprocess_mkdocs_markdown(md_content: str):
    ...
    for mermaid in mermaids:
        ...
        if ex_nihilo or changed:
            ...
            command = f"./node_modules/.bin/mmdc --input {code_file} --output {image_file} --backgroundColor transparent --scale 2 --configFile {MERMAID_CONFIG_FILE}"
...
def postprocess_tex(tex: str):
    ...
    # update figure paths
    tex = tex.replace("../media/", join('..', MKDOCS_ROOT_FOLDER, "docs/media/"))
...
if __name__ == "__main__":
    ...
    docs_folder = join(MKDOCS_ROOT_FOLDER, 'docs')
    ...
    mkdocs_config_file = join(MKDOCS_ROOT_FOLDER, 'mkdocs.yml')
